...
;(function ($, document, window, undefined) {
	...
	$.fn.userTimeout = function (opts) {
		var $defaults = {
			...
			notify: false,                      		// Toggle for notification of session ending
			timer: false,                       		// Toggle for enabling the countdown timer
			session: (sessionAlive*1000)-110000,                   		// 10 Minutes in Milliseconds, then notification of logout
		...
		var modal = function () {
			...
			if ($modalUI === 'bootstrap') {
				...
				$dialog = $('<div class="modal-dialog modal-notify modal-info" role="document">');
			...
			if ($modalUI === 'mdb') {
				console.log("Inside MDB Modal condition");
				var $container, $dialog, $content, $header, $body, $footer, $logoutBtn;

				$container = $('<div class="modal fade" id="notifyLogout" tabindex="-1" role="dialog" aria-labelledby="notifyLogoutLabel" aria-hidden="true"></div>');
				$dialog = $('<div class="modal-dialog modal-notify modal-info" role="document"></div>');
				$content = $('<div class="modal-content text-center"></div>');
				//$header = $('<div class="modal-header"><h4 class="modal-title" id="notifyLogoutLabel">' + $options.modalTitle + '</h4></div>');
				$body = $('<div class="text-center"><div class="modal-body"><i class="fa fa-bell fa-4x animated swing infinite mb-4"></i><p>' + $options.modalBody + '</p></div></div>');

				if ($options.timer === true) {
					$footer = $('<div class="modal-footer"><button type="button" id="modal-close"  class="btn btn-primary" data-dismiss="modal">' +  $options.modalStayLoggedBtn + ' (<span id="countdowntimer">' + $countDownTimer + '</span>)</button></div>');
				} else {
					$footer = $('<div class="modal-footer"><button type="button" id="modal-close" class="btn " data-dismiss="modal">' +  $options.modalStayLoggedBtn + '</button></div>');
				}

				//$logoutBtn = $('<button type="button" class="btn btn-danger" id="logoff">' + $options.modalLogOffBtn + '</button>');
